      ...
      <div className="flex w-full flex-col lg:flex-row items-center gap-20 my-20">
        ...
        <div className="flex-1">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl">
            {aboutInfo.map((item, index) => (
              ...
              >
                ...
                <h3 className={`my-3 font-semibold text-gray-700 ${isDarkTheme ? "text-gray-400" : ""}`}>
                  {item.title}
                </h3>
                <p className={`text-gray-600 text-sm ${isDarkTheme ? "text-gray-400" : ""}`}>{item.description}</p>
              </li>
            ))}
          </ul>

          <h4 className={`my-6 text-gray-700 font-Ovo ${isDarkTheme ? "text-white" : ""}`}>Tools I use :</h4>
          <ul className="flex items-center gap-3 sm:gap-5">
            ...
            <li className={`flex items-center justify-center w-12 sm:w-14 aspect-square border border-gray-400 rounded-lg cursor-pointer hover:bg-lightBlueHover hover:-translate-y-1 duration-500 ${isDarkTheme ? "hover:text-black" : "text-black"}`}>
